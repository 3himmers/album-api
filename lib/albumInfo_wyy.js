            ...
            .then((res) => {
                ...
                if (is1CD) {
                    ...
                    for (let i = 0; i < res.data.songs.length; i++) {
                        if (
                            i > 0 &&
                            res.data.songs[i].cd != res.data.songs[i - 1].cd
                        ) {
...
const searchAlbumLink = (headersCookie, ip, reqName, reqArtists, reqDate) => {
    return new Promise((resolve, reject) => {
        ...
        axios(settings)
            .then((res) => {
                ...
                for (let index = 0; index < album.length; index++) {
                    var date = new Date(album[index].publishTime);
                    Y = date.getFullYear() + "-";
                    M =
                        (date.getMonth() + 1 < 10
                            ? "0" + (date.getMonth() + 1)
                        ...
                        if (myUtils.includesENCS(name, reqName)) {
                            ...
                            for (
                                ...
                                if (
                                    ...
                                    return resolve({ code: "0", data: id });
                                }
                            }
                        }
                    }
                }
                return resolve({ code: "C0702", msg: "未在wyy搜索到该专辑" });
            })
            .catch((err) => {
                console.log(err);
                return resolve({ code: "B0011", msg: "专辑地址错误" });
