...
async function getAlbumInfo(id) {
    try {
        ...
        await spotifyApi.getAlbum(id).then(function (data) {
            body = data.body;
        });
        ...
        while (offset < body.total_tracks) {
            ...
            if (offset == 0) {
                ...
                await spotifyApi
                    .getAlbumTracks(id, { limit: 50, offset: offset })
                    .then(function (data) {
                        body2 = data.body;
                    });
            }
            for (let i = 0; i < body2.items.length; i++) {
                ...
                if (discName == null) {
                    discNameNumber =
                        body2.items[i].disc_number < 10
                            ? "0" + body2.items[i].disc_number
                            : body2.items[i].disc_number;
...
async function searchAlbumLink(reqName, reqArtists, reqDate) {
    try {
        ...
        spotifyApi.setAccessToken(spotifyToken);
        await spotifyApi.searchAlbums(reqName).then(function (data) {
        ...
        for (let index = 0; index < album.length; index++) {
            ...
            if (releaseDate == reqDate) {
                ...
                if (myUtils.includesENCS(name, reqName)) {
                    ...
                    for (
                        let index2 = 0;
                        index2 < artistsArray.length;
                        index2++
                    ) {
                        if (
                            myUtils.includesNCS(
                                reqArtists,
                                artistsArray[index2].name
                            )
                        ) {
